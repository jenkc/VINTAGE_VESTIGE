...
**Text Embeddings** (`embeddings/models.py`)
- Model: `all-MiniLM-L6-v2` (SentenceTransformers)
- Dimensionality: 384
- Input: Rich text constructed from enrichment fields
- Storage: `products.text_embedding vector(384)` column in Supabase PostgreSQL (pgvector, COSINE distance via HNSW index)

**Image Embeddings** (`embeddings/models.py`)
- Model: `clip-ViT-B-32` (SentenceTransformers)
- Dimensionality: 512
- Input: Product images (PIL Image or URL — images stored in Supabase Storage, referenced by URL)
- Storage: `products.image_embedding vector(512)` column in Supabase PostgreSQL (pgvector, COSINE distance via HNSW index)

**Separation**: The two embedding types are completely independent:
- Stored as separate columns on the same `products` table (no separate vector DB)
...
**Current Ranking** (`storage/vector_search.py`)
```python
...
- Simple cosine similarity scoring (0.0 to 1.0) via pgvector `<=>` operator
- Results sorted by similarity descending
- Both embeddings live on the same row — easy to retrieve both for cross-modal Φ calculation
- No integration measurement between modalities (yet)
...
#### Modified: `storage/vector_search.py`

Add Φ calculation to search results:

> **Supabase migration note:** With pgvector, both embeddings live as columns on
> the same `products` row. No need to retrieve from separate collections — a single
> SQL query returns both `text_embedding` and `image_embedding` alongside metadata.
> This actually makes cross-modal Φ calculation simpler than the old Qdrant setup.

```python
from iit.phi_calculator import PhiCalculator

class VectorSearch:
    def __init__(self, db: Session):
        self.db = db
        self.phi_calculator = PhiCalculator(method='simple')
    ...
    def search_with_phi(
        ...
        # Search by text similarity, but also retrieve both embeddings
        rows = self.db.execute(text("""
            SELECT id, title, platform, primary_image, era, decade,
                   vibe, fp_category, culture, ai_description,
            ...
            ORDER BY text_embedding <=> :vec
            LIMIT :limit
        """), {"vec": str(text_query_emb.tolist()), "limit": limit * 2}).fetchall()

        enriched_results = []
        for row in rows:
            row_dict = dict(row._mapping)
            text_emb = np.array(row_dict.pop('text_embedding'))
            image_emb = row_dict.pop('image_embedding')

            # Calculate Φ between text and image embeddings
            if image_emb is not None:
                phi = self.phi_calculator.calculate_phi(
                    text_emb, np.array(image_emb)
                )
            else:
                phi = 0.0

            cosine_score = row_dict['score']
...
**Note**: With pgvector, both embeddings are on the same row — no separate retrieval step needed. This is simpler and faster than the old Qdrant approach which required cross-collection lookups.
...
def search_by_complex(complex_name: str, limit: int = 10):
    ...
    # Search using normal text search (pgvector)
    results = vs.search_text(models.encode_text(query_text), limit=limit)
...
def adaptive_search(query_text: str, query_image: Optional[Image] = None, limit: int = 10):
    ...
    # Search both modalities via pgvector
    ...
    if text_emb is not None:
        text_results = vs.search_text(text_emb, limit=limit*2)
    else:
        text_results = []

    if image_emb is not None:
        image_results = vs.search_image(image_emb, limit=limit*2)
    else:
        image_results = []
...
**Deliverables**:
- `iit/phi_calculator.py` with simple Φ implementation
- Modified `storage/vector_search.py` with `search_with_phi()` (pgvector-based)
