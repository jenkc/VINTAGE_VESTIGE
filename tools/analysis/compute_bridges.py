...
def classify_temporal_type(source_era, target_era,
    ...
    # Last resort: historical vs modern platform as proxy
...
def bulk_load_embeddings(db):
    """Load all embeddings in one query. Returns dict {product_id: (text_vec, image_vec)}."""
...
def _get_embeddings(db, product_id, cache):
    """Look up embeddings from cache. Falls back to DB if missing (shouldn't happen after bulk load)."""
...
def run_opposition_pass(db, product_map, embedding_cache, existing_pairs,
                        structural_gate=0.30, top_per_pair=150):
...
def run_function_pass(db, product_map, embedding_cache, existing_pairs,
                      top_per_function=50, min_gate=0.25):
...
def compute_bridges(rebuild=False, limit=None,
    ...
    # Bulk-load all embeddings into cache (replaces lazy per-product fetches)
    embedding_cache = bulk_load_embeddings(db)

    # Detach all products from the session so they survive session cycling.
    # Force-load all attributes before expunge — any lazy field accessed after
    # expunge would raise DetachedInstanceError.
    for p in product_map.values():
        # Touch ALL fields used by Passes 2-4 to ensure they're loaded.
        # compute_structural_score uses getattr on all 15 STRUCTURAL_WEIGHTS fields,
        # plus passes access core_vibes, culture, era, decade, platform, title, vibe.
        _ = p.core_vibes, p.social_function, p.construction_technique, p.motif_family
    ...
    # Reopen session for summary queries (previous was closed after Pass 4)
    db = SessionLocal()
