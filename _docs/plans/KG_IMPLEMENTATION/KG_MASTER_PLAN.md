...
| KG is the next thing | KG starts after deploy + MMFashion pipeline complete |
...
+1 week ─── semantic_type classifier — first pass on existing bridges (~2 hours)
           └─ Populates semantic_type on all 7,324+ bridges
           └─ scripts/classify_semantic_types.py

+1 week ─── Data growth to ~1,500 enriched products
           └─ .claude/plans/functional-hopping-barto.md

+2–3 weeks ─ MMFashion pipeline (see mmfashion-bridge-plan-v3.md)
           └─ Visual attribute detection on all product images
           └─ Claude re-enrichment: MMFashion tags + controlled vibe vocabulary
           └─ Re-embed all products; recompute bridges
           └─ semantic_type classifier second pass on recomputed bridges

+4 weeks ── Deploy (Railway + Vercel)
           └─ First live URL — required before building more infrastructure
           └─ semantic_type already populated; bridge UI shows types on launch

QUICK WIN  semantic_type: already done above — no separate step needed
...
Supabase migration (Steps 12-20) ────────────────┐
                                                   ▼
semantic_type classifier (first pass) ────────────┐
                                                   ▼
Data growth to ~1,500 products ───────────────────┐
                                                   ▼
MMFashion pipeline ───────────────────────────────┐
  └─ re-enrich + re-embed + recompute bridges      │
  └─ semantic_type classifier (second pass) ───────┤
                                                   ▼
